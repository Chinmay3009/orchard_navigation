...
def match_gts_preds(gts, preds, gating=0.5):
    ...
    for row, col in zip(rows, cols):
        ...
        if cost_matrix[row, col] < gating:
            distances.append((row, distance))  # Store the GT index and distance
        else:
    ...
    return distances, rows, cols, unmatched_gts, unmatched_preds

# Function to plot matched GTs and predictions
def plot_matched_gts_preds(gts, map_json_path, row_, date_, gating=0.5):
    ...
    distances, rows, cols, missed_gts, missed_preds = match_gts_preds(gts, preds, gating=gating)

    # Print Euclidean distances for matched trees
    print("\n--- Matched Trees and Euclidean Distances ---")
    for index, distance in distances:
        print(f"Tree {index + 1}: {distance:.3f} meters")
    print("-------------------------------------------\n")

    # Plotting settings
    plt.figure(figsize=(12, 8))  # Larger figure size
    color_match = "blue"  # Line color for matched pairs
    color_gt = "green"    # Color for ground truth points
    color_pred = "red"    # Color for prediction points

    # Plot ground truths and predictions
    for i in range(len(gts)):
        if i not in rows:
            gt_point = gts[i]
            plt.scatter(gt_point[0], gt_point[1], color=color_gt, marker='o', s=60, label="Ground Truth" if i == 0 else "")

    for j in range(len(preds)):
        if j not in cols:
            pred_point = preds[j]
            plt.scatter(pred_point[0], pred_point[1], color=color_pred, marker='x', s=60, label="Prediction" if j == 0 else "")

    # Plot matched pairs with lines
    for i, (row, col) in enumerate(zip(rows, cols)):
        gt_point = gts[row]
        pred_point = preds[col]
        plt.scatter(gt_point[0], gt_point[1], color=color_gt, marker='o', s=80)  # Larger markers for matched ground truths
        plt.scatter(pred_point[0], pred_point[1], color=color_pred, marker='x', s=80)  # Larger markers for matched predictions
        plt.plot([gt_point[0], pred_point[0]], [gt_point[1], pred_point[1]], color=color_match, linestyle='--', linewidth=1)

    # Set axis limits with padding
    x_coords = [point[0] for point in gts + preds]
    y_coords = [point[1] for point in gts + preds]
    ...
    plt.title(f"Matched Ground Truths and Predictions")
...
# Define the ground truth as provided
ground_truth_trees = [
...
# Define your JSON file path and identifiers
map_json_path = "/home/chinmay/second_ros2_ws/src/row_following_pkg/tree_db_1.json"
row_ = "row11"  
date_ = "2024-11-13"  

# Call the function with ground truth data as list of tuples
plot_matched_gts_preds(ground_truth_trees, map_json_path, row_, date_, gating=0.5)
