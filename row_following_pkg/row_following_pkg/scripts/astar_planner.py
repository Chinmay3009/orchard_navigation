...
class A_STAR(Node):
    def __init__(self):
        ...
        #OG_array = np.array(Image.open('/home/chinmay/second_ros2_ws/image.pgm').convert('L')).astype(int)
    ...
    def get_path_coordinates(self):
        """
        Retrieve the path coordinates (real-world coordinates).
        """
        if not self.path_coordinates:
            self.get_logger().warn("Path coordinates not available. Run A* first.")
            return []
    ...
    def visualize_path(self, path):
        ...
        # Transform path to real-world coordinates
        ...
        # Plot occupancy grid with real-world extents
        plt.imshow(
            self.occupancy_grid,
            origin='upper',
            cmap='gray',
    ...
    def run_astar(self):
        ...
        if path:
            ...
            #self.get_logger().info("path is" (path))
...
def main(args=None):
    ...
    node.set_goal_pose(20.0, 50.5)  # Example goal (real-world coordinates)

    # Run A* repeatedly (every 2 seconds)
    node.timer = node.create_timer(2.0, node.run_astar)
