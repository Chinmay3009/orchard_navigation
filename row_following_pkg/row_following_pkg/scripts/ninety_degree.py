##WORKS PERFECT TILL THE FIRT 90 DEGREE TURN
...
class PurePursuitNode(Node):
    def __init__(self):
        ...
        self.detection_distance_threshold = 0.3  # Adjust this as needed, lower values for closer detection
    ...
    def control_loop(self):
        ...
        if self.no_trees_detected or self.check_end_of_row_early():
            if self.pass_count < 1:
                ...
                self.get_logger().info("End of second pass; initiating 90-degree right turn.")
    ...
    def check_end_of_row_early(self):
        """
        This function checks if the distance to trees in the middle line has exceeded
        a specified threshold, indicating the potential end of the row earlier.
        """
        if self.middle_line:
    ...
    def perform_90_degree_turn(self):
        if self.angle_difference(self.current_yaw, self.initial_yaw) >= (math.pi / 2) - 0.05:
            self.stop_robot()  # Stop robot after completing the right turn
    ...
    def stop_robot(self):
        ...
        twist.angular.z = 0.0
        self.vel_pub.publish(twist)
        self.get_logger().info("Robot stopped after completing both passes and the right turn.")
